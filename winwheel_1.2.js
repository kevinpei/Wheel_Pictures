		...
		if ((doPrizeDetection) && (prizes))
		{
			...
			for (x = 0; x < (prizes.length); x ++)
			{
				if ((relativeAngle >= prizes[x]['startAngle']) && (relativeAngle <= prizes[x]['endAngle']))
				{
					...
					var wintext = ("The amount you would receive from the wheel is " + prizes[x]['name'] + "\n Click Next to move to the next screen.");
					var el = document.getElementById("wintext");
					el.innerHTML = wintext;
					Qualtrics.SurveyEngine.setEmbeddedData(prizeno, prizes[x]['name']);
